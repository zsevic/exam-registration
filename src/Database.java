...
public class Database {
  ...
  public static String unpassedExams(int indeks) {
    try {
      String sql = "select distinct p.naziv,op.id_predmeta,d.indeks from obavezan_predmet op" + " join predmet p on p.id_predmeta=op.id_predmeta " + " join dosije d on d.id_smera=op.id_smera" + " where d.indeks=? and not exists(select * " + " from ispit i" + " where i.id_predmeta=op.id_predmeta and ocena>5 and " + " status_prijave='o') and p.naziv not in (" + " select naziv_predmeta from jun_2015)";
  ...
  private static ResultSet processHold(
    ResultSet rs,
    Connection con,
  ...
  private static ResultSet processHoldForUpdate(
    ResultSet rs,
    Connection con,
    PreparedStatement ps
  )
  ...
  private static ResultSet openCursorForUpdate(PreparedStatement ps)
    throws
      SQLException {
  ...
  private static ResultSet openCursor(PreparedStatement ps)
    throws
      SQLException {
    ResultSet rs = ps.executeQuery();
